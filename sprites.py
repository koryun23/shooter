...
class Player(pg.sprite.Sprite):
    def __init__(self, game):
        ...
        self.walk_r = [
            self.game.main_hero_spritesheet.get_image(x * spritesheet_width * 0.33, 0 + spritesheet_height * 0.25,
                                                      spritesheet_width * 0.33, spritesheet_height * 0.25)
            for x in range(3)
        ]
        self.walk_l = [
            self.game.main_hero_spritesheet.get_image(x * spritesheet_width * 0.33, 0 + spritesheet_height * 0.75,
                                                      spritesheet_width * 0.33, spritesheet_height * 0.25)
            for x in range(3)
        ]
...
class Enemy(pg.sprite.Sprite):
    ...
    def update(self, vel_delta_x):
        ...
        if not self.rect.colliderect(self.game.player.rect) or (
                self.rect.x + 5 + self.game.player.rect.width >= self.game.player.rect.x or self.rect.x - 5 <= self.game.player.rect.x + self.game.player.rect.width):
...
class Mummie(Enemy):
    def __init__(self, game):
        ...
        self.walk_r = [
            self.game.mummie_spritesheet.get_image(x * spritesheet_width * 0.33, 0 + spritesheet_height * 0.25,
                                                   spritesheet_width * 0.33, spritesheet_height * 0.25)
            for x in range(3)
        ]
        self.walk_l = [
            self.game.mummie_spritesheet.get_image(x * spritesheet_width * 0.33, 0 + spritesheet_height * 0.75,
                                                   spritesheet_width * 0.33, spritesheet_height * 0.25)
            for x in range(3)
        ]
...
class Zombie(Enemy):
    def __init__(self, game):
        ...
        self.walk_r = [
            self.game.zombies_spritesheet.get_image(x * spritesheet_width * 0.33, 0 + spritesheet_height * 0.25,
                                                    spritesheet_width * 0.33, spritesheet_height * 0.25)
            for x in range(3)
        ]
        self.walk_l = [
            self.game.zombies_spritesheet.get_image(x * spritesheet_width * 0.33, 0 + spritesheet_height * 0.75,
                                                    spritesheet_width * 0.33, spritesheet_height * 0.25)
            for x in range(3)
        ]
        self.current_image = self.walk_r[0]
...
class Gun(pg.sprite.Sprite):
    def __init__(self, game):
        ...
        self.dir = 1
        self.first_image = self.game.guns_spritesheet.get_image(spritesheet_width * 3 / 4, spritesheet_height * 2 / 7,
                                                                spritesheet_width / 4, spritesheet_height / 7)
...
class Sword(pg.sprite.Sprite):
    def __init__(self, game):
        ...
        self.image = pg.transform.scale(self.image,
                                        (int(self.image.get_width() * 1.2), int(self.image.get_height() * 1.2)))
        self.rect = self.image.get_rect()
        self.rect.midright = self.game.player.rect.midright
        self.vel = vec(0, 0)
        self.last_update = 0
        ...
        for i in range(len(self.images)):
            ...
            self.images[i] = pg.transform.scale(self.images[i], (
                int(self.images[i].get_width() * 1.2), int(self.images[i].get_height() * 1.2)))
